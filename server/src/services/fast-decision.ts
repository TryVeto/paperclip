...
export type FastDecisionErrorCode = "timeout" | "http_error" | "request_failed" | "invalid_response";
 ...
 */
export function resolveFastDecisionConfig(env: FastDecisionEnvironment, agentId: string): FastDecisionConfig | null {
    if (!isExplicitlyTrue(env.PAPERCLIP_FAST_DECISION_ENABLED))
        return null;
    ...
    if (!allowlist.includes(agentId))
        return null;
    const endpoint = resolveResponsesEndpoint(env.PAPERCLIP_FAST_DECISION_BASE_URL, isExplicitlyTrue(env.PAPERCLIP_FAST_DECISION_ALLOW_REMOTE));
    ...
    if (!model)
        throw configError("model_missing", "Fast decision model is required");
    ...
    if (!provider)
        throw configError("provider_missing", "Fast decision accounting provider is required");
...
export async function executeFastDecision(config: FastDecisionConfig, input: FastDecisionInput, options: ExecuteFastDecisionOptions = {}): Promise<FastDecisionResult> {
    ...
    let wireResponse: { response: Response; payload: unknown } | undefined;
    try {
        ...
        wireResponse = await Promise.race([
            requestPromise,
            timeoutPromise,
        ]) as { response: Response; payload: unknown };
    }
    catch (error) {
...
function buildRequestBody(input: any, config: any) {
    const cappedPayload = {
...
function resolveResponsesEndpoint(rawBaseUrl: any, allowRemote: any) {
    const baseUrl = rawBaseUrl?.trim();
    if (!baseUrl)
        throw configError("base_url_missing", "Fast decision base URL is required");
    let parsed;
    try {
    ...
    if (!allowRemote && !isLoopback) {
        throw configError("remote_endpoint_not_allowed", "Fast decision base URL must be loopback unless remote access is explicitly enabled");
    }
...
function isLoopbackHostname(rawHostname: any) {
...
function isExplicitlyTrue(value: any) {
    const normalized = value?.trim().toLowerCase();
    return normalized === "true" || normalized === "1";
}
function configError(code: any, message: any) {
    return new FastDecisionConfigError(code, message);
}
function cap(value: any, limit: any) {
    return value.length <= limit ? value : value.slice(0, limit);
}
function parseDecisionPayload(payload: any) {
...
function extractOutputCandidates(payload: any) {
    if (!isRecord(payload))
        return [];
    const candidates = [];
...
function validateDecision(value: any) {
    if (!isRecord(value))
        return null;
    ...
    if (typeof value.action !== "string" || !(FAST_DECISION_ACTIONS as readonly string[]).includes(value.action)) {
        return null;
    }
    if (typeof value.comment !== "string" || value.comment.length > FAST_DECISION_COMMENT_LIMIT)
        return null;
    ...
    if (value.action === "comment_and_done" && value.comment.trim().length === 0)
        return null;
    return {
...
function parseUsage(payload: any) {
...
function readTokenCount(value: any) {
    return typeof value === "number" && Number.isSafeInteger(value) && value >= 0 ? value : 0;
}
function readNonEmptyString(value: any, key: any) {
    if (!isRecord(value))
        return undefined;
